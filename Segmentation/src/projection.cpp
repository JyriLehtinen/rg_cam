...
void construct_model(std::vector<cv::Point3f> *model)
{
	...
	(*model).push_back(cv::Point3f(0.0f, 0.0f, 640.0f));
	(*model).push_back(cv::Point3f(823.0f, 0.0f, 640.0f));
	(*model).push_back(cv::Point3f(823.0f, 0.0f, 0.0f));
	(*model).push_back(cv::Point3f(823.0f, 0.0f, -640.0f));
	(*model).push_back(cv::Point3f(0.0f, 0.0f, -640.0f));
	(*model).push_back(cv::Point3f(0.0f, 0.0f, 0.0f));
}
...
*/
cv::Mat project_points(cv::Mat _camera, cv::Mat _distorsion, cv::Mat _rvec, cv::Mat _tvec, cv::Mat *image=NULL)
{
	std::vector<cv::Point2f> image_points;
	std::vector<cv::Point3f> posts;
	
	posts.push_back(cv::Point3f(-137.2f, 0.0f, 1189.0f));
	posts.push_back(cv::Point3f(960.2f, 0.0f, 1189.0f));
	posts.push_back(cv::Point3f(-228.6f, 0.0f, 0.0f));
	posts.push_back(cv::Point3f(1051.6f, 0.0f, 0.0f));
	cv::projectPoints(posts, _rvec, _tvec, _camera, _distorsion, image_points);	
	...
	else
	{
		...
		{
			cv::circle(*image, image_points[i], 10, cv::Scalar(0,0,255), 4);
		}
	}
